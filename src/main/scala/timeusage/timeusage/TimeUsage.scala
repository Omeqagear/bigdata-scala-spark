...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType ={
    val first = StructField(columnNames.head, DataTypes.StringType, nullable = false)
    val others = columnNames.drop(1) map (StructField(_, DataTypes.DoubleType, nullable = false))
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row(line.head :: line.drop(1).map(_.toDouble):_*)
  ...
  def timeUsageSummary(
    ...
    def sum_udf(colums: List[Column], sum: Column ): Column =colums match {
      case Nil => sum
