...
object WikipediaRanking {
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    ...
    val articlePairs = rdd.flatMap(article => {
      val mention = langs.filter(lang => article.text.split(" ").contains(lang))
      mention.map(lang => (lang, article))
    })

    articlePairs.groupByKey
  }
